...
describe("discoverSkills", () => {
  ...
  it("buildSkillsXmlBlock canonical format (Bedrock cache-key stability)", () => {
    ...
    expect(block).toMatchInlineSnapshot(`
      ...
      - \`h2o-field-brief\` — Render requested opportunity artifacts — Field Brief, Playbook, Analytical Read, Proposal Shell — one artifact tool at a time. For full opportunity packages, produce them sequentially in priority order (Field Brief first, then Playbook, Analytical Read, Proposal Shell). For explicit single-artifact requests, produce only the requested artifact. Content depth scales with deal stage. Also produces three lightweight non-PDF outputs on demand: follow-up email drafts, site-visit prep checklists, and objection responses. Uses brand kit primitives. Compose, don't re-reason — consume the upstream positioning record verbatim. Trigger after h2o-positioning or explicit artifact request. Does not produce markdown mirrors. Does not run on focused conversational questions (those use the fast path in the system prompt).
